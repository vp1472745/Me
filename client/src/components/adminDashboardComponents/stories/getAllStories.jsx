...
import { Calendar, MapPin, Users, Music, Video, Image as ImageIcon } from "lucide-react";

const StoriesList = ({ stories, loading, error, onRetry, onStoryClick }) => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="mt-4 text-[#8b7355] font-medium">Loading beautiful stories...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="text-center py-20 bg-white/40 rounded-2xl backdrop-blur-sm">
        ...
        <button
          onClick={onRetry}
          className="px-6 py-2.5 bg-[#8b7355] text-white rounded-full hover:bg-[#6b5b4b] transition shadow-md hover:shadow-lg"
        >
          Try Again
        </button>
      </div>
      ...
      {stories.map((story, index) => (
        ...
        >
          ...
          <div className="relative overflow-hidden h-72">
            <img
              src={story.coverImage || "https://images.unsplash.com/photo-1519741497674-611481863552?w=1200"}
              alt={story.title}
